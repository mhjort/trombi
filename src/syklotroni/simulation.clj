(ns syklotroni.simulation
  (:require [syklotroni.simulation-runners :as runners]
            [syklotroni.schema :as schema]
            [syklotroni.progress-tracker :as progress-tracker]
            [syklotroni.simulation-util :refer [arg-count
                                                 choose-runner
                                                 clean-result
                                                 log-exception
                                                 weighted-scenarios]]
            [schema.core :refer [validate]]
            [syklotroni.timers :as timers]
    ...
    (let [return {:name (:name step)
      ...
      (if (= c response)
        ...
        [(assoc return :end (now)
                       :exception (ex-info "syklotroni: request timed out" {:timeout-in-ms timeout})
                       :result false
